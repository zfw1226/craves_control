...
def transform(pt, center, scale, res, invert=0, rot=0):
    ...
    if invert:
        t = np.linalg.inv(t)
    new_pt = np.array([pt[0] - 2, pt[1] - 1, 1.]).T
...
def multi_scale_merge(imgs, scales):
    ...
    new_img = np.zeros(
        [int(imgs.shape[2] * sf / 2) * 2, int(imgs.shape[3] * sf / 2) * 2, imgs.shape[1]])  # create a big image
    ...
    for i in range(imgs.shape[0]):  # paste all images to the big one
        ...
        for j in range(imgs.shape[1]):
            ...
            new_img[c[0] - new_ht:c[0] + new_ht, c[1] - new_wd:c[1] + new_wd, j] = new_img[c[0] - new_ht:c[0] + new_ht,
                                                                                   c[1] - new_wd:c[1] + new_wd,
                                                                                   j] + img_resized

        counter[c[0] - new_ht:c[0] + new_ht, c[1] - new_wd:c[1] + new_wd, :] = counter[c[0] - new_ht:c[0] + new_ht,
                                                                               c[1] - new_wd:c[1] + new_wd, :] + 1.0
